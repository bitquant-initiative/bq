...
public class DependencyTest {
  ...
  private void checkNotArtifact(File pom, JsonNode dep, String artifactId) {
    Assertions.assertThat(dep.path("artifactId").asText())
        .withFailMessage("%s should not contain dependency: %s", pom, artifactId)
        .isNotEqualTo(artifactId);
  }
    ...
    jd.forEach(
        dep -> {
          ...
          if (dep.path("groupId").asText().equals("io.github.bitquant-initiative")) {
            Assertions.assertThat(dep.path("version").asText())
                .withFailMessage(
                    "%s: %s should have version: ${revision}", pom, dep.path("artifactId").asText())
                .isEqualTo("${revision}");
          }
        });
  }
